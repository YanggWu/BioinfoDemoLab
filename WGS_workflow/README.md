...
2. **检查 BWA 索引文件**：
   - 脚本会检查参考基因组目录下是否存在以下 BWA 索引文件：
     - `*.bwt`、`*.ann`、`*.amb`、`*.pac`、`*.sa`
   - 如果不存在，脚本将自动运行 `bwa index` 创建索引。

3. **检查 Samtools 索引文件**：
   - 脚本会检查是否存在参考基因组的 `.fai` 索引文件。
   - 如果不存在，脚本将自动运行 `samtools faidx` 创建索引。
...
**目的**：使用 Picard 标记并去除 PCR 或光学重复的 reads。
...
- **去除重复后的 BAM 文件**：
  - `[样本名]_dup.bam`
- **重复序列指标文件**：
  - `[样本名]_dup_metrics.txt`
- **MultiQC 报告**：
  - `[样本名]_multiqc_report` 目录
- **原始变异结果文件**：
  - `[样本名]_raw.vcf`
- **过滤后的变异结果文件**：
  - `[样本名]_filter.vcf`
...
8. **重要提示**：在第 2 步的 BWA-MEM 比对中，脚本中可能存在一个小错误，输入的 FASTQ 文件应为 `${fq1_clean}` 和 `${fq2_clean}`，请确认脚本中这一部分是否正确。
   ...
   bwa mem \
     ...
     ${BWA_INDEX} ${fq1_clean} ${fq2_clean} | \
     samtools sort -@ ${nt} -o $sortedbam
   ```
...
- **作者**：wuyang
- **日期**：2024 年 11 月 8 日
- **邮箱**：wuyang@example.com

---

希望本指南能帮助您顺利运行基因组重测序分析流程，如有疑问，请随时联系。

---

**注**：在使用脚本前，请仔细阅读并理解各步骤，必要时根据自己的数据和需求进行调整。